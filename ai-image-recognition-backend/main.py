    ...
    try:
        ...
        try:
            if tool == "object_detection":
                # 调用带可视化的检测方法
                annotations, annotated_image_base64 = ai_service.detect_objects_with_visualization(image_bytes)
            elif tool == "image_classification":
                # 调用分类方法
                classification_results = ai_service.classify_image(temp_file_path)
                annotations = classification_results
            elif tool == "image_segmentation":
                # 调用分割方法
                segmentation_results = ai_service.segment_objects(temp_file_path)
