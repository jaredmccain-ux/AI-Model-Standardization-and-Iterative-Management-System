  ...
  <div class="settings-page">
    ...
    <el-card class="settings-form-card" style="margin-top: 20px;">
      <el-form :model="settings" label-width="150px" v-loading="loading">
        <el-form-item label="模型训练输出路径">
          <div style="display: flex; width: 100%; gap: 10px;">
            <el-input v-model="settings.training_output_path" placeholder="例如: runs/train" style="flex: 1;">
               <template #append>
                  <el-button :icon="Folder" @click="selectOutputFolder('training_output_path')" title="选择文件夹" />
               </template>
            </el-input>
        ...
        <el-form-item label="模型评估保存路径">
          <div style="display: flex; width: 100%; gap: 10px;">
            <el-input v-model="settings.evaluation_save_path" placeholder="例如: runs/evaluation" style="flex: 1;">
               <template #append>
                  <el-button :icon="Folder" @click="selectOutputFolder('evaluation_save_path')" title="选择文件夹" />
               </template>
            </el-input>
            <el-button type="primary" @click="saveSetting('evaluation_save_path')">保存</el-button>
          </div>
          <div class="form-tip">指定模型评估结果（指标、报告、图表）的默认保存目录。支持绝对路径或相对路径。</div>
        </el-form-item>
...
const saveSetting = async (key) => {
  try {
    ...
    } else if (key === 'evaluation_save_path') {
      description = '模型评估结果保存路径';
    }
