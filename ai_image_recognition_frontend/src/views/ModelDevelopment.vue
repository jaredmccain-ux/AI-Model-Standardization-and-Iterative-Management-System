      ...
      <el-col :span="12">
        ...
        <el-card class="status-card" style="margin-bottom: 20px;">
          ...
          <div v-if="currentTask">
            ...
            <div style="margin-top: 16px;" v-if="currentTask.status === 'running' || isCancelling">
              <el-button type="danger" @click="cancelTraining" :loading="isCancelling" :disabled="isCancelling">
...
onMounted(() => {
  ...
  if (proj?.id) {
    trainingConfig.project_id = proj.id
    if (!trainingConfig.name) trainingConfig.name = proj.name || ''
    axios.get(`${getApiUrl()}/api/projects/${proj.id}`).then(resp => {
      const datasetYaml = resp.data?.dataset_yaml_path
      if (datasetYaml && !trainingConfig.data_path) {
        trainingConfig.data_path = datasetYaml
      }
    }).catch(() => {})
  }
})
