 ...
 * @param {Array} globalCategories - 全局类别列表 (用于 YOLO 格式确保 ID 一致)
 * @returns {Object} - 包含 content, url, mimeType, extension
 */
export const exportAnnotationData = (data, format = 'json', globalCategories = null) => {
  ...
  switch (format) {
    ...
    case 'yolo':
      content = convertToYOLO(data, globalCategories);
      mimeType = 'text/plain';
      extension = 'txt';
      break;
 ...
 * @param {Array} globalCategories - 全局类别列表
 * @returns {String} - YOLO格式的文本字符串
 */
function convertToYOLO(data, globalCategories = null) {
  const categories = globalCategories || [...new Set(data.annotations.map(ann => ann.label || 'unknown'))];
  let yoloContent = '';
  
  data.annotations.forEach(annotation => {
    if (!isBoxAnnotation(annotation)) return;
    const bbox = annotation.bbox;
    let categoryId = categories.indexOf(annotation.label || 'unknown');
    
    // 如果找不到类别（可能使用了全局列表但当前标签不在其中），暂时归为 0 或跳过
    if (categoryId === -1) {
        console.warn(`Label ${annotation.label} not found in categories list.`);
        categoryId = 0; 
    }
...
/**
 * 生成 YOLO 训练所需的 dataset.yaml 内容
 * @param {Array} categories - 类别名称列表
 * @param {String} datasetPath - 数据集根路径 (可选)
 * @returns {String} - YAML 内容
 */
export const generateDatasetYAML = (categories, datasetPath = '') => {
  let content = '';
  // 如果没有提供路径，使用占位符
  content += `path: ${datasetPath || '请修改为数据集根目录的绝对路径'} # dataset root dir\n`;
  content += `train: images/train # train images (relative to 'path')\n`;
  content += `val: images/val # val images (relative to 'path')\n`;
